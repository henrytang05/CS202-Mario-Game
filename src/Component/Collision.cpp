...
CollisionComponent::~CollisionComponent() { entities = nullptr; }
...
bool CollisionComponent::DynamicRectVsRect(const float deltaTime,
...
bool CollisionComponent::ResolveDynamicRectVsRect(
    const float deltaTime, Shared<AbstractEntity> r_static) {
...
void CollisionComponent::update(float deltaTime) {
  ...
  for (int i = 0; i < (int)entities->size(); i++) {
    if (*otherEntity[i] == *entity)
      continue;
    Vector2 position =
        otherEntity[i]->getComponent<PositionComponent>().getPosition();
    Vector2 size =
        otherEntity[i]->getComponent<BoundingBoxComponent>().getSize();
...
Shared<AbstractEntity> CollisionComponent::getBelow() { return contact[2]; }
Shared<AbstractEntity> CollisionComponent::getAbove() { return contact[0]; }
Shared<AbstractEntity> CollisionComponent::getRight() { return contact[3]; }
Shared<AbstractEntity> CollisionComponent::getLeft() { return contact[1]; }
...
void CollisionComponent::setEntities(
    Shared<std::vector<Shared<AbstractEntity>>> _entities) {
  entities = _entities;
}
