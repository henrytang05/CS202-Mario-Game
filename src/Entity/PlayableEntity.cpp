...
// }
//
// PlayableEntity::PlayableEntity() : fallAcc(GRAVITY_DEC),
// state(make_shared<DroppingState>("SMALL", "RIGHT")), isDeath(false) {
//   addComponent<PlayerTag>();
// }
...
//
//   // resolve collision
//   Shared<AbstractEntity> above =
//   getComponent<CollisionComponent>().getAbove(); Shared<AbstractEntity> below
//   = getComponent<CollisionComponent>().getBelow(); Shared<AbstractEntity>
//   left = getComponent<CollisionComponent>().getLeft(); Shared<AbstractEntity>
//   right = getComponent<CollisionComponent>().getRight(); if(above) {
//     if(above->name == "BrokenBlock") {
//       if(state->getSize() == "SMALL")
//         above->getComponent<BlockTriggerComponent>().setTrigger(new
//         TriggerBrokenBlockWhenHitBySmall(above->getComponent<PositionComponent>().getPosition()));
...
//       state = make_shared<DroppingState>(state->getSize(),
//       state->getFacing());
...
//           state =
//               make_shared<SkiddingState>(state->getSize(),
//               state->getFacing());
//         } else {
...
//       fallAcc = FALL_ACC;
//       state = make_shared<JumpingState>(state->getSize(),
//       state->getFacing()); if (state->getSize() == "SMALL")
...
//       state = make_shared<DucklingState>(state->getSize(),
//       state->getFacing());
...
//       state = make_shared<StandingState>(state->getSize(),
//       state->getFacing());
...
//     if (std::fabs(velocity.x) >= MIN_WALKING_VELO) {
//           state = make_shared<MovingState>(state->getSize(),
//           state->getFacing());
//     } else {
//       state = make_shared<StandingState>(state->getSize(),
//       state->getFacing());
