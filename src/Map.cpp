...
void MapRenderer::addTileset(const std::string &tilesetPath,
                             const std::string &imagePath, int firstGid) {
...
std::vector<Shared<AbstractEntity>>
MapRenderer::createMap(const std::string &mapPath) {
  ...
  for (const auto &tileset : mapData["tilesets"]) {
    ...
    std::string imagePath =
        tilesetPath.substr(0, tilesetPath.size() - 5) + ".png";
...
void MapRenderer::loadLayer(const json &layer) {
  ...
  for (const auto &tileIdValue : layer["data"]) {
    if (!tileIdValue.is_null() && tileIdValue.is_number()) {
      ...
      auto obj = entityFactory->createBlock(textureMap[tileId].first,
                                            textureMap[tileId].second,
                                            {(float)x, (float)y});
      if (obj) {
...
void MapRenderer::loadObjectGroup(const json &layer) {
  ...
  for (const auto &object_layer : object_layers) {
    ...
    if (name == "Pipe") {
      ...
      auto obj = entityFactory->createEnemy(position, size);
      if (obj) {
        objects.push_back(obj);
      }
    } else {
