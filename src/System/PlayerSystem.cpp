...
void PlayerSystem::update(float dt) {
    ...
    for(auto tEntity : Entities) {
        ...
        if (state.getState() != "JUMPING" && state.getState() != "DROPPING") {
            ...
            if (IsKeyPressed(KEY_UP)) {
                ...
                if (state.getSize() == "SMALL")
                    tEntity.lock()->getComponent<MarioSoundComponent>().PlayJumpSmallEffect();
                else if (state.getSize() == "LARGE")
                    tEntity.lock()->getComponent<MarioSoundComponent>().PlayJumpSuperEffect();
            }
            ...
            if (keyDown && std::fabs(velocity.x) < playerConstant.MIN_WALKING_VELO &&
                ...
                tEntity.lock()->getComponent<BoundingBoxComponent>().setSize({16.0f, 16.0f});
            ...
            if (IsKeyReleased(KEY_DOWN) && state.getState() == "DUCKLING") {
                ...
                tEntity.lock()->getComponent<BoundingBoxComponent>().setSize({16.0f, 28.0f});
                tEntity.lock()->getComponent<PositionComponent>().setPosition(
                    {position.x,
                    position.y - 13.0f});
            }
        ...
        if (velocity.x <= -playerConstant.MAX_RUNNING_VELO)
        velocity.x = -playerConstant.MAX_RUNNING_VELO;
        if (velocity.x >= playerConstant.MAX_WALKING_VELO && !IsKeyDown(KEY_LEFT_SHIFT))
        ...
        tEntity.lock()->getComponent<TextureComponent>().changeState(state.getCurrentState());
