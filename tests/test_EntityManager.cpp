...
TEST_F(EntityMangerTest, testAddDuplicateComponent) {
    ...
    ASSERT_THROW(em.addComponent<TestComponent>(entity.lock(), 5), std::runtime_error);
}
...
TEST_F(EntityMangerTest, testEntityHasAllComponents2) {
    ...
    bool hasAll = em.hasAllComponents<TestComponent, TestComponent2>(entity.lock());
    ASSERT_TRUE(hasAll);
}
...
TEST_F(EntityMangerTest, testGetComponent3) {
    ASSERT_THROW(em.getComponent<TestComponent>(entity.lock()), std::runtime_error);
}
...
TEST_F(EntityMangerTest, testRemoveComponent2) {
    ASSERT_THROW(em.removeComponent<TestComponent>(entity.lock()), std::runtime_error);
}
...
TEST_F(EntityMangerTest, testModifyComponent4) {
    ASSERT_THROW(
        em.modifyComponent<TestComponent>(entity.lock(), 10), std::runtime_error
    );
}
